...
const FunnelEditorNavigation = ({
  ...
  const handleOnBlurTitleChange: FocusEventHandler<HTMLInputElement> = async (
    ...
    if (event.target.value) {
      ...
      toast('Success', {
        description: 'Titre de la page du funnel enregistré',
      })
      router.refresh()
    } else {
      toast('Oppse!', {
        description: 'Vous devez avoir un titre !',
      })
  ...
  const handleOnSave = async () => {
    ...
    try {
      ...
      await saveActivityLogsNotification({
        ...
        description: `Funnel page modifier | ${response?.name}`,
        subAccountId: subaccountId,
      })
      toast('Success', {
        description: 'Éditeur enregistré',
      })
    } catch (error) {
      toast('Oppse!', {
        description: "Impossible d'enregistrer l'éditeur",
      })
    }
  }
        ...
        <aside className="flex items-center gap-2">
          ...
          <div className="flex flex-col item-center mr-4">
            ...
            <span className="text-muted-foreground text-xs">
              Derniéres modification {funnelPageDetails.updatedAt.toLocaleDateString()}
            </span>
          </div>
